...
import { FileText, Download, Printer, Plus, X, Save, Link2, CreditCard, Search } from 'lucide-react';
...
interface FinancialsProps {
  initialTab?: 'Invoices' | 'Quotations' | 'Receipts';
}
...
export const Financials: React.FC<FinancialsProps> = ({ initialTab = 'Invoices' }) => {
  ...
  // Sync state when tab changes or external updates happen
  useEffect(() => {
    setInvoices(getInvoices());
  }, [activeTab, isModalOpen]);

  // Logic to auto-fill from rental
  ...
  // Logic to auto-fill from pending invoice (for Receipts)
  const handleInvoiceSelect = (invoiceId: string) => {
      ...
      if (invoice) {
          ...
          setHasVat(false); // VAT already calculated on invoice
      }
  };
  ...
  const handleCreate = (e: React.FormEvent) => {
      ...
      // If creating a receipt linked to an invoice, update the invoice status
      if (activeTab === 'Receipts' && selectedInvoiceToPay) {
          markInvoiceAsPaid(selectedInvoiceToPay);
      }

      setInvoices(getInvoices());
      setIsModalOpen(false);
      setFormData({ clientId: '', items: [], date: new Date().toISOString().split('T')[0], status: 'Pending', contractId: '', paymentMethod: 'Bank Transfer', paymentReference: '' });
  ...
  const downloadPDF = (doc: Invoice) => {
      ...
      if (client) {
          ...
          alert(`Could not generate PDF: Client data missing for ID ${doc.clientId}`);
      }
  };
  ...
  const initiatePayment = (invoice: Invoice) => {
      ...
      setIsModalOpen(true);
      // Timeout to allow modal to render and state to settle before setting form data
  ...
  const filteredDocs = invoices.filter(i => {
      ...
      if (activeTab === 'Invoices') matchesType = i.type === 'Invoice';
      else if (activeTab === 'Quotations') matchesType = i.type === 'Quotation';
      else if (activeTab === 'Receipts') matchesType = i.type === 'Receipt'; 
      ...
      const matchesSearch = 
        ...
        (i.paymentReference && i.paymentReference.toLowerCase().includes(searchLower));
      ...
      <div className="space-y-8 animate-fade-in">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-slate-500 font-medium">Create invoices, manage VAT, and track payment history</p>
          </div>
          <div className="flex gap-4">
              <div className="relative group w-64 hidden sm:block">
                  <Search className="absolute left-3 top-3 text-slate-400 group-focus-within:text-slate-800 transition-colors" size={18} />
                  <input 
                    type="text" 
                    value={searchTerm}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm text-slate-600">
              <thead className="bg-slate-50/50 border-b border-slate-100">
                <tr>
                  <th className="px-6 py-4 font-bold text-xs uppercase text-slate-400 tracking-wider">ID</th>
                  <th className="px-6 py-4 font-bold text-xs uppercase text-slate-400 tracking-wider">Date</th>
                  <th className="px-6 py-4 font-bold text-xs uppercase text-slate-400 tracking-wider">Client / Info</th>
                  {activeTab === 'Receipts' && (
                      <>
                          <th className="px-6 py-4 font-bold text-xs uppercase text-slate-400 tracking-wider">Method</th>
                          <th className="px-6 py-4 font-bold text-xs uppercase text-slate-400 tracking-wider">Ref #</th>
                      </>
                  )}
                  <th className="px-6 py-4 font-bold text-xs uppercase text-slate-400 tracking-wider text-right">Total</th>
                  <th className="px-6 py-4 font-bold text-xs uppercase text-slate-400 tracking-wider text-center">Status</th>
                  <th className="px-6 py-4 font-bold text-xs uppercase text-slate-400 tracking-wider text-center">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {filteredDocs.length > 0 ? filteredDocs.map((doc) => (
                  <tr key={doc.id} className="hover:bg-slate-50 transition-colors">
                    <td className="px-6 py-4 font-bold text-slate-900">{doc.id}</td>
                    <td className="px-6 py-4">{doc.date}</td>
                    <td className="px-6 py-4">
                      <div className="flex flex-col">
                          <span className="text-xs font-bold text-slate-700">{mockClients.find(c => c.id === doc.clientId)?.companyName || 'Unknown Client'}</span>
                          {doc.contractId && <span className="text-[10px] text-indigo-500 font-medium flex items-center gap-1"><Link2 size={10}/> Contract {doc.contractId}</span>}
                      </div>
                    </td>
                    {activeTab === 'Receipts' && (
                      <>
                          <td className="px-6 py-4 text-xs">{doc.paymentMethod || '-'}</td>
                          <td className="px-6 py-4 text-xs font-mono">{doc.paymentReference || '-'}</td>
                      </>
                    )}
                    ...
                    <td className="px-6 py-4 flex justify-center gap-2">
                      <button onClick={() => downloadPDF(doc)} className="p-2 text-slate-400 hover:text-slate-900 bg-slate-50 hover:bg-slate-200 rounded-lg transition-colors" title="Download PDF">
                        <Download size={16} />
                      </button>
                      {/* Show Payment Button if Invoice is Pending */}
                    ...
                    <tr>
                        <td colSpan={activeTab === 'Receipts' ? 8 : 6} className="px-6 py-12 text-center text-slate-400 italic">No documents found.</td>
                    </tr>
                )}
              </tbody>
            </table>
              ...
              <form onSubmit={handleCreate} className="p-8 space-y-6">
                ...
                {activeTab === 'Receipts' && (
                    <div className="p-4 bg-green-50 rounded-xl border border-green-100 mb-2">
                         <MinimalSelect 
                            ...
                            options={[{value: '', label: 'Select Invoice to Pay...'}, ...getInvoices().filter(i => i.status === 'Pending' && i.type === 'Invoice').map(i => ({
                                value: i.id, label: `Inv #${i.id} - $${i.total} (${mockClients.find(c => c.id === i.clientId)?.companyName})`
                            }))]}
                        />
                    </div>
                )}
                ...
                {activeTab !== 'Receipts' && (
                    <div className="p-4 bg-indigo-50 rounded-xl border border-indigo-100 mb-2">
                        <MinimalSelect 
                            ...
                            options={[{value: '', label: 'Select Rental to Auto-fill...'}, ...getContracts().map(c => {
                                const client = mockClients.find(cl => cl.id === c.clientId);
                                const billboard = getBillboards().find(b => b.id === c.billboardId);
                                return {value: c.id, label: `${client?.companyName} - ${billboard?.name} (${c.details})`};
                            })]}
                        />
                    </div>
                )}
                ...
                {activeTab === 'Receipts' && (
                    <div className="grid grid-cols-2 gap-6">
                        ...
                        />
                        <MinimalInput label="Reference Number" value={formData.paymentReference} onChange={(e: any) => setFormData({...formData, paymentReference: e.target.value})} />
                    </div>
                )}
                ...
                <div className="bg-slate-50 rounded-2xl p-6 space-y-4 border border-slate-100">
                    ...
                    <div className="flex gap-3">
                        <div className="flex-1">
                            <MinimalInput label="Description" value={newItem.description} onChange={(e: any) => setNewItem({...newItem, description: e.target.value})} />
                        </div>
                        <div className="w-32">
                            <MinimalInput label="Amount ($)" type="number" value={newItem.amount} onChange={(e: any) => setNewItem({...newItem, amount: Number(e.target.value)})} />
                        </div>
                ...
                <div className="flex items-center gap-2">
                    <input 
                        type="checkbox" 
                        checked={hasVat} 
                        disabled={activeTab === 'Receipts' && !!selectedInvoiceToPay}
                ...
                <button type="submit" className="w-full py-4 text-white bg-slate-900 rounded-xl hover:bg-slate-800 flex items-center justify-center gap-2 shadow-xl font-bold uppercase tracking-wider transition-all">
                  <Save size={18} /> Create {activeTab.slice(0, -1)}
                </button>
              </form>
            </div>
          </div>
