...
export const Financials: React.FC<FinancialsProps> = ({ initialTab = 'Invoices' }) => {
  ...
  const handleInvoiceSelect = (invoiceId: string) => {
      ...
      if (invoice) { 
          setFormData({ ...formData, clientId: invoice.clientId, contractId: invoice.contractId, items: [{ description: `Payment for Invoice #${invoice.id}`, amount: invoice.total }] }); 
          setHasVat(false); 
      }
  };
  ...
  const handleCreate = (e: React.FormEvent) => {
      ...
      const prefix = activeTab === 'Quotations' ? 'QT' : activeTab === 'Receipts' ? 'RCT' : 'INV';
      const docType = activeTab === 'Invoices' ? 'Invoice' : activeTab === 'Quotations' ? 'Quotation' : 'Receipt';
      const docStatus = activeTab === 'Receipts' ? 'Paid' : 'Pending';
