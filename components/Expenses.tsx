...
export const Expenses: React.FC = () => {
  ...
  // Printing Job State
  const [newJob, setNewJob] = useState<Partial<PrintingJob>>({
    clientId: '',
    description: '',
    dimensions: '',
    pvcCost: 0,
  ...
  const handleAddExpense = (e: React.FormEvent) => {
      ...
      addExpense(expense);
      setGeneralExpenses(getExpenses()); // Refresh
  ...
  const exportExpenseReport = () => {
    ...
    const csvHeader = "Client,Total Jobs,Total Billed,Total Internal Cost,Net Profit,Margin %\n";
    const csvRows = clients.map(client => {
        const jobs = mockPrintingJobs.filter(j => j.clientId === client.id);
        ...
        const margin = totalSpent > 0 ? ((profit / totalSpent) * 100).toFixed(2) : '0';

        return `"${client.companyName}",${jobs.length},${totalSpent},${totalCost},${profit},${margin}`;
    ...
    link.href = url;
    link.setAttribute('download', `printing_expenses_report_${new Date().toISOString().slice(0,10)}.csv`);
  ...
  const calculateTotalJobCost = () => {
    return (newJob.pvcCost || 0) + (newJob.inkCost || 0) + (newJob.electricityCost || 0) + (newJob.operatorCost || 0) + (newJob.weldingCost || 0);
  };
    ...
    <>
      <div className="space-y-6 animate-fade-in">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h2 className="text-4xl font-extrabold tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-slate-900 to-slate-600 mb-2">Expenses & Production</h2>
            <p className="text-slate-500 font-medium">Internal cost tracking, printing jobs, and profitability analysis</p>
          </div>
          
          {activeTab === 'Printing' && (
              <button onClick={() => setIsAddJobModalOpen(true)} className="bg-slate-900 text-white px-5 py-2.5 rounded-full text-sm font-bold uppercase tracking-wider shadow-lg hover:shadow-xl hover:scale-105 transition-all flex items-center gap-2">
                  <Plus size={16} /> New Print Job
              </button>
          )}
          {activeTab === 'General' && (
              <button onClick={() => setIsAddExpenseModalOpen(true)} className="bg-slate-900 text-white px-5 py-2.5 rounded-full text-sm font-bold uppercase tracking-wider shadow-lg hover:shadow-xl hover:scale-105 transition-all flex items-center gap-2">
                  <Plus size={16} /> New Expense
              </button>
          )}
          {activeTab === 'Reports' && (
              <button onClick={exportExpenseReport} className="bg-slate-100 text-slate-600 px-5 py-2.5 rounded-full text-sm font-bold uppercase tracking-wider hover:bg-slate-200 transition-all flex items-center gap-2">
        ...
        <div className="flex border-b border-slate-200 gap-8">
          <button onClick={() => setActiveTab('General')} className={`pb-3 text-sm font-bold uppercase tracking-wider transition-all border-b-2 ${activeTab === 'General' ? 'border-slate-900 text-slate-900' : 'border-transparent text-slate-400 hover:text-slate-600'}`}>General Expenses</button>
          <button onClick={() => setActiveTab('Printing')} className={`pb-3 text-sm font-bold uppercase tracking-wider transition-all border-b-2 ${activeTab === 'Printing' ? 'border-slate-900 text-slate-900' : 'border-transparent text-slate-400 hover:text-slate-600'}`}>Printing Module</button>
          <button onClick={() => setActiveTab('Reports')} className={`pb-3 text-sm font-bold uppercase tracking-wider transition-all border-b-2 ${activeTab === 'Reports' ? 'border-slate-900 text-slate-900' : 'border-transparent text-slate-400 hover:text-slate-600'}`}>Cost Reports</button>
        </div>

        {activeTab === 'General' && (
            <div className="bg-white shadow-sm rounded-2xl border border-slate-100 overflow-hidden animate-fade-in">
                <div className="p-6 border-b border-slate-50">
                    <h3 className="text-lg font-semibold text-slate-800">Operational Expenses</h3>
                </div>
                <table className="w-full text-left text-sm text-slate-600">
                    <thead className="bg-slate-50/50">
                        <tr>
                            <th className="px-6 py-4 font-bold text-xs uppercase text-slate-400 tracking-wider">Date</th>
                            <th className="px-6 py-4 font-bold text-xs uppercase text-slate-400 tracking-wider">Category</th>
                            <th className="px-6 py-4 font-bold text-xs uppercase text-slate-400 tracking-wider">Description</th>
                    ...
                    <tbody className="divide-y divide-slate-50">
                        {generalExpenses.map(exp => (
                            <tr key={exp.id} className="hover:bg-slate-50 transition-colors">
                                <td className="px-6 py-4 font-mono text-xs">{exp.date}</td>
                                <td className="px-6 py-4">
                                    <span className={`px-2 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${
                                        exp.category === 'Maintenance' ? 'bg-orange-50 text-orange-600' :
                                        exp.category === 'Electricity' ? 'bg-yellow-50 text-yellow-600' :
                                ...
                                <td className="px-6 py-4 text-xs font-mono text-slate-400">{exp.reference || '-'}</td>
                                <td className="px-6 py-4 text-right font-bold text-slate-900">${exp.amount.toLocaleString()}</td>
                            </tr>
                        ))}
                        {generalExpenses.length === 0 && (
                            <tr><td colSpan={5} className="px-6 py-8 text-center text-slate-400 italic">No expenses recorded yet.</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
        ...
        {activeTab === 'Printing' && (
          <div className="space-y-6 animate-fade-in">
            ...
            <div className="bg-white shadow-sm rounded-2xl border border-slate-100 overflow-hidden">
                <div className="p-6 border-b border-slate-50">
                    <h3 className="text-lg font-semibold text-slate-800">Recent Printing Jobs</h3>
                </div>
                <table className="w-full text-left text-sm text-slate-600">
                    <thead className="bg-slate-50/50">
                      <tr>
                          ...
                          <th className="px-6 py-4 font-bold text-xs uppercase text-slate-400 tracking-wider">Job Details</th>
                          <th className="px-6 py-4 font-bold text-xs uppercase text-slate-400 tracking-wider text-right">Cost</th>
                          <th className="px-6 py-4 font-bold text-xs uppercase text-slate-400 tracking-wider text-right">Charged</th>
                          <th className="px-6 py-4 font-bold text-xs uppercase text-slate-400 tracking-wider text-right">Profit</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                      {mockPrintingJobs.map(job => {
                          const profit = job.chargedAmount - job.totalCost;
                          return (
                            <tr key={job.id} className="hover:bg-slate-50 transition-colors">
                                <td className="px-6 py-4">{job.date}</td>
                                <td className="px-6 py-4 font-medium text-slate-900">{getClientName(job.clientId)}</td>
                                <td className="px-6 py-4">
                                  <p className="font-medium text-slate-800">{job.description}</p>
                                  <p className="text-xs text-slate-500">{job.dimensions}</p>
                                </td>
                                <td className="px-6 py-4 text-right">${job.totalCost}</td>
                                <td className="px-6 py-4 text-right">${job.chargedAmount}</td>
        ...
        {activeTab === 'Reports' && (
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-8 animate-fade-in">
              <h3 className="text-xl font-bold text-slate-800 mb-6">Client Printing Spend Report</h3>
              <div className="space-y-4">
                {getClients().map(client => {
                    ...
                    return (
                      <div key={client.id} className="border border-slate-100 rounded-xl p-6 hover:shadow-md transition-all">
                          <div className="flex justify-between items-center mb-4">
                            <h4 className="font-bold text-slate-900 text-lg">{client.companyName}</h4>
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            ...
                            <div>
                                <p className="text-xs text-slate-400 font-bold uppercase">Our Cost</p>
                                <p className="text-xl font-bold text-slate-700">${totalCost}</p>
                            </div>
                            <div className="md:col-span-2">
                                <p className="text-xs text-slate-400 font-bold uppercase mb-1">Margin Analysis</p>
                                <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
                                    <div className="h-full bg-green-500" style={{ width: `${((totalSpent - totalCost) / totalSpent) * 100}%` }}></div>
                                </div>
                            </div>
                          </div>
                      </div>
            ...
            <div className="bg-white/95 backdrop-blur-xl rounded-3xl shadow-2xl max-w-2xl w-full border border-white/20">
               ...
               <form onSubmit={handleAddJob} className="p-8 space-y-8">
                  <div className="grid grid-cols-2 gap-6">
                     ...
                     <div className="col-span-2">
                        <MinimalInput label="Description / Ref" value={newJob.description} onChange={(e: any) => setNewJob({...newJob, description: e.target.value})} />
                     </div>
                  ...
                  <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100 space-y-4">
                     ...
                     <div className="grid grid-cols-3 gap-4">
                        ...
                        <div className="flex flex-col justify-end">
                           ...
                           <p className="text-lg font-bold text-slate-800">${calculateTotalJobCost()}</p>
                        </div>
                     </div>
                  </div>
