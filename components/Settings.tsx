...
import { getUsers, addUser, updateUser, deleteUser, getAuditLogs, getCompanyLogo, setCompanyLogo, getCompanyProfile, updateCompanyProfile, RELEASE_NOTES, resetSystemData, createSystemBackup, restoreSystemBackup, getLastManualBackupDate, getAutoBackupStatus } from '../services/mockData';
import { User, Shield, Building, ScrollText, Download, Plus, X, Save, Phone, MapPin, Edit2, Trash2, AlertTriangle, Cloud, Upload, History, RefreshCw, Database, FileUp, FileDown, Clock, Archive } from 'lucide-react';
...
export const Settings: React.FC = () => {
  ...
  const [backupStatus, setBackupStatus] = useState({ manual: getLastManualBackupDate(), auto: getAutoBackupStatus() });

  useEffect(() => {
      // Refresh backup status when entering Data tab
      if (activeTab === 'Data') {
          setBackupStatus({ manual: getLastManualBackupDate(), auto: getAutoBackupStatus() });
      }
  }, [activeTab]);
  ...
  const handleLogoUpload = (e: React.ChangeEvent<HTMLInputElement>) => { 
      ...
      if (file) { 
          ...
          reader.onloadend = () => {
              ...
              setNewLogoData(base64); // Store for saving later
          };
