...
import { FileText, Calendar, CheckCircle, AlertCircle, Printer, Wrench, Download, X, Eye, Clock } from 'lucide-react';
...
export const ContractList: React.FC = () => {
  ...
  const getBillingDay = (dateStr: string) => {
      if (!dateStr) return '';
              ...
              <div className="flex items-start gap-5">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 text-sm text-slate-500 mt-1">
                    ...
                    <span className={`font-bold px-2 py-0.5 rounded text-xs ${contract.side === 'A' || contract.side === 'B' ? 'bg-blue-50 text-blue-700' : 'bg-purple-50 text-purple-700'}`}>
                      {contract.details}
                    </span>
                  </div>
                  <div className="flex items-center gap-3 mt-3 text-xs text-slate-400 uppercase tracking-wide font-medium flex-wrap">
                    ...
                    <span className="flex items-center gap-1 text-emerald-700 bg-emerald-50 px-2 py-1 rounded-md border border-emerald-100 shadow-sm" title="Monthly Billing Cycle">
                        <Clock size={12} /> Bill Day: {getBillingDay(contract.startDate)}
                    </span>
          ...
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center z-50 p-4 transition-all animate-fade-in">
            <div className="bg-white/95 backdrop-blur-xl rounded-3xl shadow-2xl max-w-lg w-full border border-white/20">
              ...
              <div className="p-8 space-y-6">
                 <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100 flex justify-between items-center">
                     ...
                     <div className="text-right">
                        ...
                        <span className="inline-flex items-center gap-1 px-3 py-1 bg-white border border-slate-200 rounded-lg text-sm font-bold text-slate-800 shadow-sm">
                            <Clock size={14} className="text-emerald-500"/> {getBillingDay(selectedContract.startDate)} of month
                        </span>
                     </div>
                 </div>
                 ...
                 <button onClick={() => handleDownload(selectedContract)} className="w-full py-4 text-white bg-slate-900 rounded-xl hover:bg-slate-800 flex items-center justify-center gap-2 shadow-xl font-bold uppercase tracking-wider transition-all">
...
// Simple Icon component for the New Contract button
