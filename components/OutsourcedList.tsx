...
export const OutsourcedList: React.FC = () => {
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (currentBillboard.id) {
        ...
        // Refresh local state manually or by refetching
  ...
  const handleDeleteConfirm = () => {
      if(itemToDelete) {
          ...
          setBillboards(prev => prev.filter(b => b.id !== itemToDelete.id));
    ...
    <>
      <div className="space-y-8 animate-fade-in">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h2 className="text-4xl font-extrabold tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-slate-900 to-slate-600 mb-2">Outsourced Inventory</h2>
            <p className="text-slate-500 font-medium">Assign existing billboards to 3rd party partners</p>
          </div>
          <button 
              onClick={openAdd}
          ...
          >
              <Plus size={18} /> Assign Outsourced
          </button>
        </div>
              ...
              <div key={billboard.id} className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 hover:shadow-lg transition-all group hover:-translate-y-1 duration-300">
                  <div className="flex justify-between items-start mb-4">
                      <div className="flex items-center gap-3">
                          ...
                          <div>
                              <h3 className="font-bold text-slate-900 leading-tight">{billboard.billboardName}</h3>
                              <p className="text-xs text-slate-500 font-mono">ID: {billboard.billboardId}</p>
                          </div>
                      </div>
                      <div className="flex gap-2">
                          <button onClick={() => { setCurrentBillboard(billboard); setIsModalOpen(true); }} className="text-slate-300 hover:text-slate-600 transition-colors">
                              <Edit2 size={16} />
                          </button>
                          <button onClick={() => setItemToDelete(billboard)} className="text-slate-300 hover:text-red-500 transition-colors">
                              <Trash2 size={16} />
                          </button>
                      </div>
                  </div>
                  ...
                  <div className="space-y-4 py-4 border-t border-slate-50">
                      <div>
                          <p className="text-[10px] uppercase font-bold text-slate-400 mb-1">Media Owner (Partner)</p>
                          <p className="text-sm font-medium text-slate-800">{billboard.mediaOwner}</p>
                          <p className="text-xs text-slate-500">{billboard.ownerContact}</p>
                      </div>
                      
                      <div className="grid grid-cols-2 gap-4">
                          <div className="bg-green-50 p-3 rounded-xl border border-green-100">
                              <div className="flex items-center gap-2 text-green-700 text-xs font-bold uppercase mb-1">
                                  <DollarSign size={12} /> Payout/Mo
                              </div>
                              <p className="text-lg font-bold text-slate-900">${billboard.monthlyPayout.toLocaleString()}</p>
                          </div>
                          <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                              <div className="flex items-center gap-2 text-slate-500 text-xs font-bold uppercase mb-1">
                                  <Calendar size={12} /> Ends
                              </div>
                              <p className="text-sm font-bold text-slate-800">{billboard.contractEnd}</p>
                          </div>
                      </div>
                  </div>
              </div>
          ...
          <div className="bg-white/95 backdrop-blur-xl rounded-3xl shadow-2xl max-w-lg w-full border border-white/20">
            ...
            <form onSubmit={handleSave} className="p-8 space-y-6">
              <MinimalSelect 
                  ...
                  options={[{value: '', label: 'Select Asset...'}, ...mockBillboards.map(b => ({value: b.id, label: `${b.name} (${b.type})`}))]}
              />
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center z-50 p-4 transition-all">
          <div className="bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl max-w-sm w-full border border-white/20 p-6 text-center">
             ...
             <p className="text-slate-500 mb-6 text-sm">
               Are you sure you want to remove the outsourced assignment for <span className="font-bold text-slate-700">{itemToDelete.billboardName}</span>?
             </p>
