...
export const BillboardList: React.FC = () => {
  ...
  return (
    <>
      ...
      {isMapShareModalOpen && (<div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center z-50 p-4 transition-all animate-fade-in"><div className="bg-white/95 backdrop-blur-2xl rounded-3xl shadow-2xl max-w-sm w-full border border-white/20 p-8 transform scale-100"><div className="w-16 h-16 bg-gradient-to-br from-indigo-50 to-indigo-100 rounded-2xl flex items-center justify-center mx-auto mb-6 text-indigo-600 shadow-inner border border-indigo-200"><Share2 size={32} /></div><h3 className="text-2xl font-bold text-slate-900 mb-2 text-center">Share Map</h3><p className="text-slate-500 text-center text-sm mb-8 leading-relaxed">Clients will see a simplified view without pricing details.</p><div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 flex items-center justify-between mb-8 shadow-inner"><code className="text-xs text-slate-600 truncate max-w-[200px] font-mono">app.spiritus.com/map/view</code><button onClick={copyMapLink} className="p-2 hover:bg-white rounded-xl text-slate-500 transition-all hover:text-indigo-600 hover:shadow-sm"><Copy size={18}/></button></div><div className="flex gap-4"><button onClick={() => setIsMapShareModalOpen(false)} className="flex-1 py-4 text-slate-600 bg-white border border-slate-200 hover:bg-slate-50 rounded-xl font-bold uppercase text-xs tracking-wider transition-all">Close</button><button onClick={() => { setIsClientView(true); setViewMode('map'); setIsMapShareModalOpen(false); }} className="flex-1 py-4 text-white bg-slate-900 hover:bg-slate-800 rounded-xl font-bold uppercase text-xs tracking-wider transition-all shadow-lg shadow-slate-900/20">Preview</button></div></div></div>)}
      {isAddModalOpen && (<div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center z-50 p-4 transition-all overflow-y-auto"><div className="bg-white/95 backdrop-blur-2xl rounded-[2rem] shadow-2xl max-w-3xl w-full border border-white/20 my-8 transform scale-100"><div className="p-8 border-b border-slate-100 flex justify-between items-center"><h3 className="text-2xl font-bold text-slate-900 tracking-tight">Add New Asset</h3><button onClick={() => setIsAddModalOpen(false)} className="p-2 hover:bg-slate-100 rounded-full transition-colors"><X size={24} className="text-slate-400" /></button></div><form onSubmit={handleAddBillboard} className="p-8 grid grid-cols-1 md:grid-cols-2 gap-8"><div className="space-y-6"><MinimalInput label="Name" value={newBillboard.name} onChange={(e: any) => setNewBillboard({...newBillboard, name: e.target.value})} required /><MinimalInput label="Location Description" value={newBillboard.location} onChange={(e: any) => setNewBillboard({...newBillboard, location: e.target.value})} required /><MinimalSelect label="Town / City" value={newBillboard.town} onChange={(e: any) => setNewBillboard({...newBillboard, town: e.target.value})} options={ZIM_TOWNS.map(t => ({ value: t, label: t }))}/><MinimalSelect label="Type" value={newBillboard.type} onChange={(e: any) => setNewBillboard({...newBillboard, type: e.target.value})} options={[{ value: BillboardType.Static, label: 'Static (Side A/B)' }, { value: BillboardType.LED, label: 'LED (Slots)' }]}/> <MinimalTextArea label="Visibility & Traffic Analysis" value={newBillboard.visibility || ''} onChange={(e: any) => setNewBillboard({...newBillboard, visibility: e.target.value})}/></div><div className="space-y-6"><div className="space-y-3"><p className="text-xs font-bold uppercase text-slate-400 tracking-wider">Asset Image</p><div className="flex items-center gap-4"><div className="w-28 h-28 bg-slate-50 rounded-2xl overflow-hidden border border-slate-200 flex items-center justify-center shadow-inner">{newBillboard.imageUrl ? <img src={newBillboard.imageUrl} className="w-full h-full object-cover"/> : <ImageIcon className="text-slate-300 w-8 h-8" />}</div><label className="flex-1 cursor-pointer group"><div className="h-28 border-2 border-dashed border-slate-200 rounded-2xl flex flex-col items-center justify-center text-slate-400 group-hover:border-indigo-300 group-hover:text-indigo-500 transition-colors bg-slate-50/50 group-hover:bg-indigo-50/30"><span className="text-xs font-bold uppercase tracking-wider mb-1">Click to Upload</span><span className="text-[10px]">JPG, PNG (Max 5MB)</span></div><input type="file" className="hidden" accept="image/*" onChange={(e) => handleImageUpload(e, false)} /></label></div></div><div className="grid grid-cols-2 gap-4"><MinimalInput label="Width (m)" type="number" value={newBillboard.width} onChange={(e: any) => setNewBillboard({...newBillboard, width: Number(e.target.value)})} /><MinimalInput label="Height (m)" type="number" value={newBillboard.height} onChange={(e: any) => setNewBillboard({...newBillboard, height: Number(e.target.value)})} /></div><div className="grid grid-cols-2 gap-4"><MinimalInput label="Latitude" type="number" value={newBillboard.coordinates?.lat} onChange={(e: any) => setNewBillboard({...newBillboard, coordinates: {...newBillboard.coordinates!, lat: Number(e.target.value)}})} /><MinimalInput label="Longitude" type="number" value={newBillboard.coordinates?.lng} onChange={(e: any) => setNewBillboard({...newBillboard, coordinates: {...newBillboard.coordinates!, lng: Number(e.target.value)}})} /></div>{newBillboard.type === BillboardType.Static ? (<div className="space-y-4 pt-2"><p className="text-xs font-bold uppercase text-slate-400 tracking-wider">Monthly Rates</p><div className="grid grid-cols-2 gap-4"><MinimalInput label="Side A Rate ($)" type="number" value={newBillboard.sideARate} onChange={(e: any) => setNewBillboard({...newBillboard, sideARate: Number(e.target.value)})} /><MinimalInput label="Side B Rate ($)" type="number" value={newBillboard.sideBRate} onChange={(e: any) => setNewBillboard({...newBillboard, sideBRate: Number(e.target.value)})} /></div></div>) : (<div className="space-y-4 pt-2"><p className="text-xs font-bold uppercase text-slate-400 tracking-wider">LED Configuration</p><div className="grid grid-cols-2 gap-4"><MinimalInput label="Total Slots" type="number" value={newBillboard.totalSlots} onChange={(e: any) => setNewBillboard({...newBillboard, totalSlots: Number(e.target.value)})} /><MinimalInput label="Rate / Slot ($)" type="number" value={newBillboard.ratePerSlot} onChange={(e: any) => setNewBillboard({...newBillboard, ratePerSlot: Number(e.target.value)})} /></div></div>)}</div><div className="md:col-span-2 pt-6 flex gap-4 border-t border-slate-100"><button type="button" onClick={() => setIsAddModalOpen(false)} className="flex-1 py-4 text-slate-500 hover:bg-slate-50 rounded-xl font-bold uppercase text-xs tracking-wider transition-colors">Cancel</button><button type="submit" className="flex-1 py-4 text-white bg-slate-900 rounded-xl hover:bg-slate-800 shadow-lg font-bold uppercase tracking-wider transition-all hover:scale-[1.02]">Create Asset</button></div></form></div></div>)}
      {editingBillboard && (<div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center z-50 p-4 transition-all overflow-y-auto"><div className="bg-white/95 backdrop-blur-2xl rounded-[2rem] shadow-2xl max-w-2xl w-full border border-white/20 my-8"><div className="p-8 border-b border-slate-100 flex justify-between items-center"><h3 className="text-2xl font-bold text-slate-900 tracking-tight">Edit Asset</h3><button onClick={() => setEditingBillboard(null)} className="p-2 hover:bg-slate-100 rounded-full transition-colors"><X size={24} className="text-slate-400" /></button></div><form onSubmit={handleSaveEdit} className="p-8 grid grid-cols-1 md:grid-cols-2 gap-8"><div className="space-y-6"><MinimalInput label="Name" value={editingBillboard.name} onChange={(e: any) => setEditingBillboard({...editingBillboard, name: e.target.value})} /><MinimalSelect label="Town" value={editingBillboard.town} onChange={(e: any) => setEditingBillboard({...editingBillboard, town: e.target.value})} options={ZIM_TOWNS.map(t => ({value:t, label:t}))} /><div className="grid grid-cols-2 gap-4"><MinimalInput label="Latitude" type="number" value={editingBillboard.coordinates?.lat} onChange={(e: any) => setEditingBillboard({...editingBillboard, coordinates: {...editingBillboard.coordinates!, lat: Number(e.target.value)}})} /><MinimalInput label="Longitude" type="number" value={editingBillboard.coordinates?.lng} onChange={(e: any) => setEditingBillboard({...editingBillboard, coordinates: {...editingBillboard.coordinates!, lng: Number(e.target.value)}})} /></div><MinimalTextArea label="Visibility & Traffic Analysis" value={editingBillboard.visibility || ''} onChange={(e: any) => setEditingBillboard({...editingBillboard, visibility: e.target.value})}/></div><div className="space-y-6"><div className="space-y-3"><p className="text-xs font-bold uppercase text-slate-400 tracking-wider">Asset Image</p><div className="flex items-center gap-4"><div className="w-28 h-28 bg-slate-50 rounded-2xl overflow-hidden border border-slate-200 flex items-center justify-center shadow-inner">{editingBillboard.imageUrl ? <img src={editingBillboard.imageUrl} className="w-full h-full object-cover"/> : <ImageIcon className="text-slate-300" />}</div><label className="flex-1 cursor-pointer group"><div className="h-28 border-2 border-dashed border-slate-200 rounded-2xl flex flex-col items-center justify-center text-slate-400 group-hover:border-indigo-300 group-hover:text-indigo-500 transition-colors bg-slate-50/50 group-hover:bg-indigo-50/30"><span className="text-xs font-bold uppercase tracking-wider mb-1">Change Photo</span><span className="text-[10px]">JPG, PNG</span></div><input type="file" className="hidden" accept="image/*" onChange={(e) => handleImageUpload(e, true)} /></label></div></div>{editingBillboard.type === BillboardType.Static ? (<div className="grid grid-cols-2 gap-6 pt-2"><MinimalInput label="Side A Monthly ($)" type="number" value={editingBillboard.sideARate} onChange={(e: any) => setEditingBillboard({...editingBillboard, sideARate: Number(e.target.value)})} /><MinimalInput label="Side B Monthly ($)" type="number" value={editingBillboard.sideBRate} onChange={(e: any) => setEditingBillboard({...editingBillboard, sideBRate: Number(e.target.value)})} /></div>) : (<div className="grid grid-cols-2 gap-6 pt-2"><MinimalInput label="Total Slots" type="number" value={editingBillboard.totalSlots} onChange={(e: any) => setEditingBillboard({...editingBillboard, totalSlots: Number(e.target.value)})} /><MinimalInput label="Rate / Slot ($)" type="number" value={editingBillboard.ratePerSlot} onChange={(e: any) => setEditingBillboard({...editingBillboard, ratePerSlot: Number(e.target.value)})} /></div>)}<button type="submit" className="w-full py-4 text-white bg-slate-900 rounded-xl hover:bg-slate-800 flex items-center justify-center gap-2 shadow-xl font-bold uppercase tracking-wider transition-all mt-8">Save Changes</button></div></form></div></div>)}
      {billboardToDelete && (<div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center z-50 p-4 transition-all"><div className="bg-white/95 backdrop-blur-2xl rounded-3xl shadow-2xl max-w-sm w-full border border-white/20 p-8 text-center"><div className="w-20 h-20 bg-rose-50 rounded-full flex items-center justify-center mx-auto mb-6 border-4 border-rose-100 text-rose-500"><AlertTriangle size={36} /></div><h3 className="text-2xl font-bold text-slate-900 mb-2">Delete Asset?</h3><p className="text-slate-500 mb-8 font-medium">Are you sure you want to delete <span className="font-bold text-slate-800">{billboardToDelete.name}</span>?</p><div className="flex gap-3"><button onClick={() => setBillboardToDelete(null)} className="flex-1 py-4 text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-xl font-bold uppercase text-xs tracking-wider transition-colors">Cancel</button><button onClick={handleConfirmDelete} className="flex-1 py-4 text-white bg-rose-500 hover:bg-rose-600 rounded-xl font-bold uppercase text-xs tracking-wider transition-colors shadow-lg shadow-rose-500/30">Delete</button></div></div></div>)}
    </>
  );
};
